...
pub enum PeerError {
    ...
    #[error("Peer {} sent an invalid or unexpected response", .0)]
    InvalidResponse(SocketAddr), //TODO Add expected response type

    #[error("Peer {} had bloom filter disabled", .0)]
    PeerBloomDisabled(SocketAddr),

    #[error("Internal Mutex poisoning error")]
    MutexPoisoned,

    #[error("Internal Mutex wait timed out")]
    MutexTimedout,

    #[error("Internal RW read lock poisoned")]
    RwReadLockPoisined,

    #[error("Internal RW write lock poisoned")]
    RwWriteLockPoisoned,

    #[error("Mempool Mutex poisoned")]
    MempoolPoisoned,
